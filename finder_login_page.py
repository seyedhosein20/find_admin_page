...
import os
inp = input("Enter url for lohin page : ").replace("http://","").replace("https://","").replace("www.","").replace("/","")
inp = inp+"/"
os.system("cls")
file_1 = open("adminlist.txt","r+")
list2 = file_1.read(28000)
list2 = list2.split()
file_nam = "true.txt"
...
try:
    ...
    for i in list2:
         ...
         http_= requests.get("http://"+inp+i).status_code
         
         if http_ ==200:
            print(Fore.GREEN+inp+i+Fore.GREEN+"  =======>>sucsess!")
            f = open("true.txt","a+")
            f.write(inp+i+"\n")
            f.close()

         else:
           print(Fore.WHITE+inp+i+Fore.RED+"  =====>>>no page ")
